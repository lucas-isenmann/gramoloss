...
export class Graph<V extends Vertex,L extends Link, S extends Stroke, A extends Area> {
    ...
    static directed_from_list_default<V extends Vertex,L extends Link, S extends Stroke, A extends Area>(l: Array<[number,number]>, vertex_default: ()=> V, arc_default: (x: number, y: number) => L ): Graph<V,L,S,A>{
        const g = new Graph<V,L,S,A>();
        const indices = new Set<number>();
        for ( const [x,y] of l.values()){
            if (indices.has(x) == false){
